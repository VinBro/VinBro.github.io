...
const lifePathMap = new Map<number, string>([
...
function compute() {
    const birthDateElement = document.getElementById('birthDate') as HTMLInputElement | null;
    let birthDateStr: string | null = null;
    if (birthDateElement) {
        birthDateStr = birthDateElement.value;
    }

    const dateToCompareToElement = document.getElementById('dateToCompareTo') as HTMLInputElement | null;
    let dateToCompareToStr: string | null = null;
    ...
    let errorMessage: string = validate(birthDateStr, "La date de naissance n'est pas valide \n");
    ...
    let lifePathResult: number = lifePath(birthDateStr!);
    ...
    let yearOfLifeResult: number = yearOfLife(currentYear.toString(), birthDateStr!);
    ...
    let firstNameLettersMeaning: string = createCharsSignifications(firstName!);

    let textForChatGPT: string = questionIntroForChatgpt + "<br>" + lifePathFirstSentence + "<br>" + lifePathMeaning + "<br>" + yearOfLifeMeaning + "<br>" + firstNameLettersMeaning;
...
function validate(inputText: string | null | undefined, errorMessage: string): string {
...
function computeCurrentYear(dateToCompareToStr: string, birthDateStr: string): number {
    const dateToCompareTo = new Date(dateToCompareToStr!);
    const birthDate = new Date(birthDateStr!);
    let currentYear: number = dateToCompareTo.getFullYear();
...
function createCharsSignifications(firstName: string): string {
    let result: string = "";
    ...
    if (ul) {
        ...
        let alreadyUsedLetters = new Set<string>();
        for (const letter of firstName) {
            let lowerCaseLetter: string = letter.toLocaleLowerCase();
...
function yearOfLife(currentYear: string, birthDate: string): number {
    let result: number = 0;
    result += computeSumOfText(currentYear);

    let splittedBirthDate: string[] = birthDate.split("-");
...
function computeSumOfText(text: string): number {
    let result: number = 0;
...
function setDateToday() {
    ...
    const dateToCompareToElement = document.getElementById('dateToCompareTo') as HTMLInputElement;
